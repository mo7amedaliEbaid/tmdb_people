        ...
        builder: (context, state) {
          ...
          if (state is PopularPeopleLoaded) {
            return ListView.builder(
              ...
              itemCount:
                  state.people.length +
                  (state.hasReachedMax ? 0 : 1), // 👈 show loader if more data
              itemBuilder: (_, i) {
                ...
                return ListTile(title: Text(state.people[i].name));
              },
            );
          }
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  ElevatedButton(
                    onPressed: () => context.read<PopularPeopleBloc>().add(
                      FetchPopularPeople(),
                    ),
                    child: const Text('Retry'), // 👈 needed for your test
                  ),
                ],
              ),
            );
        ...
        builder: (context, state) {
          ...
          if (state is PopularPeopleLoaded) {
            return ListView.builder(
              ...
              itemBuilder: (_, i) => Text(state.people[i].name),
            );
          }
          if (state is PopularPeopleError) {
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    bloc = PopularPeopleBloc(
      getPopularPeople: GetPopularPeople(widget.repository),
      getCachedPopularPeople: GetCachedPopularPeople(widget.repository),
    );
    bloc.add(FetchPopularPeople());
